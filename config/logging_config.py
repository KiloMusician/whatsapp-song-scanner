...
log_dir = Path(str(APP_CONFIG["data_dir"])) / "logs"
...
def setup_logging(name: Optional[str] = None, level: Optional[str] = None):
    ...
    """
    log_level_value = (level or APP_CONFIG["log_level"]) or "INFO"
    log_level = str(log_level_value)
    ...
    file_handler = logging.handlers.RotatingFileHandler(
        log_dir / "app.log", maxBytes=100 * 1024 * 1024, backupCount=10  # 100MB
    )
    ...
    error_handler = logging.handlers.RotatingFileHandler(
        log_dir / "error.log", maxBytes=100 * 1024 * 1024, backupCount=10  # 100MB
    )
    ...
    scan_handler = logging.handlers.RotatingFileHandler(
        log_dir / "scan_history.log", maxBytes=50 * 1024 * 1024, backupCount=5  # 50MB
    )
