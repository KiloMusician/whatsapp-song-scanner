...
class MusicBrainzClient:
    ...
    def __init__(self):
        ...
        self.rate_limiter = RateLimiter(
            calls=1,
            period=MUSIC_MATCHING_CONFIG['musicbrainz']['rate_limit_per_second']
        )
    ...
    def search_song(self, song_title: str, artist_name: str = None) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Found {len(formatted_results)} results for: {song_title}")
            ...
            logger.error(f"MusicBrainz response error: {e}")
            return []
