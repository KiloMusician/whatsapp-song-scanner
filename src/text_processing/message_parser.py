...
class MessageParser:
    ...
    def __init__(self):
        ...
        self.patterns = [
            # "play Song by Artist"
            (re.compile(r'play\s+["\']?(?P<title>[^"\']+?)["\']?\s+by\s+(?P<artist>[^"\'.]+)', re.IGNORECASE), 'play_by'),
            
            # "request: Artist - Song"
            (re.compile(r'request:?\s+(?P<artist>[^-]+)\s*-\s*(?P<title>[^"\'.]+)', re.IGNORECASE), 'request_dash'),
            
            # "can you play: Song"
            (re.compile(r'can\s+you\s+play:?\s+["\']?(?P<title>[^"\']+)["\']?', re.IGNORECASE), 'can_you_play'),
            
            # "song? Title"
            (re.compile(r'song\??\s+["\']?(?P<title>[^"\']+)["\']?', re.IGNORECASE), 'song_question'),
            
            # "I want to hear Song by Artist"
            (re.compile(r'(?:i\s+)?want\s+to\s+hear\s+["\']?(?P<title>[^"\']+?)["\']?\s+by\s+(?P<artist>[^"\'.]+)', re.IGNORECASE), 'want_to_hear'),
            
            # "Artist - Song"
            (re.compile(r'^(?P<artist>[^-]+)\s*-\s*(?P<title>[^"\'.]+)$', re.IGNORECASE), 'simple_dash'),
        ...
        for pattern, method_name in self.patterns:
            ...
            if match:
                ...
                if candidate['title']:
                    ...
                    logger.debug(f"Extracted candidate: {candidate}")
