    ...
    try:
        ...
        for result in results:
            ...
            request = RequestOperations.create_request(
                db, chat_id=str(chat_id), matched_song_id=match.id, requested_by=username
            )
...
def process_playlist(text, chat_id):
    ...
    send_message(
        chat_id,
        f"🎧 <b>Scanning {platform.replace('_', ' ').title()} playlist...</b>\n\nThis may take a moment.",
        "HTML",
    )
...
def format_response(
    results, radiodj_queued=None, radiodj_found_not_queued=None, radiodj_queue_details=None
):
            ...
            if "message" in update:
                ...
                if playlist_info:
                    ...
                    if result:
                        ...
                        matched_count = len(
                            [m for m in result.get("matches", []) if m.get("matched_title")]
                        )
                ...
                if results:
                    ...
                    try:
                        ...
                        for item in persisted:
                            ...
                            request = (
                                db.query(SongRequest).filter(SongRequest.id == request.id).first()
                            )
                            ...
                            if request and sync_service.process_request(db, request):
                                ...
                                if request.radiodj_track_id:
                                    queued_track = radiodj_client.get_track_by_id(
                                        request.radiodj_track_id
                                    )
                                    if queued_track:
